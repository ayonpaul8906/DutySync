...
export const TaskCard: React.FC<TaskCardProps> = ({ passenger, pickup, drop, tourLocation, status, onStart, onComplete, task, onAction }) => {
  const isAssigned = status === "assigned";
  const isInProgress = status === "in-progress";

  const displayPassenger = passenger || task?.passenger?.name || task?.passengerName || "Passenger";
  const location = tourLocation || task?.tourLocation || (pickup && drop ? `${pickup} → ${drop}` : (pickup || drop || ""));
  ...
  return (
    <div className="bg-white rounded-[24px] border border-slate-100 shadow-sm overflow-hidden mb-6">
      <div className="p-5">
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800 text-lg">{displayPassenger}</span>
          </div>
          ...
          <div className="flex-1">
            <div>
              <div className="flex justify-between items-center">
                <p className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">Tour Location</p>
                <Navigation2 size={16} className="text-blue-600 cursor-pointer" />
              </div>
              <p className="text-slate-700 font-medium truncate">{location}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {isAssigned && (
        <button 
          onClick={() => onStart ? onStart() : onAction?.(task)}
      ...
      {isInProgress && (
        <button 
          onClick={() => onComplete ? onComplete() : onAction?.(task)}
